...
function controller_detail()
{
	...
	if ($_SESSION['isadmin'] === true)
		...
		$query = mysql_query("SELECT * FROM product WHERE id='{$id}' AND status=1") or die (mysql_error());
...
function controller_autocompletesearch()
{
	...
	$data = mysql_query("SELECT DISTINCT name FROM product WHERE name LIKE '%{$k}%' AND status=1 ORDER BY name");
	$res = array();
...
function controller_wishlist()
{
	...
	{
		$_SESSION["lastest_wishlist_id"] = $_GET["pid"];
	...
	foreach ($wishlist as $w)
	{
		if ($w["type"] == "product")
		{
			$data = mysql_query("SELECT DISTINCT p . * , pc.category_type
				...
				WHERE p.id =  '".$w["id"]."' AND p.status =1
				LIMIT 1");
